...
export function DigitalLeaseSigning() {
  ...
  return (
    <Card className="w-full max-w-md" aria-label="Digital Lease Signing">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="border border-warm-gray-200 rounded-lg p-4 bg-warm-gray-50">
          ...
          {/* Signature Area */}
          <div
            className="border-2 border-dashed border-warm-gray-300 rounded p-4 text-center cursor-pointer"
            onClick={!isComplete ? handleSign : undefined}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => {
            ...
            aria-label={isComplete ? "Document signed" : "Click to sign document"}
          >
            {isComplete ? (
              <div className="space-y-2">
                <div className="text-2xl font-script text-teal-600 select-none">
                  {/* Signature placeholder */}
                  <svg width="120" height="40" viewBox="0 0 120 40" className="mx-auto" aria-hidden="true" focusable="false">
                    <path 
                ...
                <div className="text-xs text-warm-gray-600 select-none">
                  Signed on {new Date().toLocaleDateString()}
                </div>
              </div>
            ) : (
              <div className="space-y-2">
                <Pen className="h-6 w-6 text-warm-gray-400 mx-auto" aria-hidden="true" />
                <div className="text-sm text-warm-gray-600">Click to sign</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Progress value={signingProgress} className="h-2" aria-valuemin={0} aria-valuemax={100} aria-valuenow={signingProgress} />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 text-sm">
            {isComplete ? (
              ...
              <div className="h-4 w-4 border-2 border-warm-gray-300 rounded-full" aria-hidden="true" />
            )}
        ...
        <div className="space-y-3">
          {!isComplete ? (
            ...
            <Button 
              variant="outline"
              className="w-full"
              aria-label="Download Signed Document Copy"
            >
