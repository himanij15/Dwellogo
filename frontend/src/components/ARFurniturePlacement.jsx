...
export function ARFurniturePlacement() {
  ...
  const [placedItems, setPlacedItems] = useState(["2"]);
  const [isARActive, setIsARActive] = useState(false);

  const toggleItemPlacement = (itemId) => {
    setPlacedItems(prev => 
  ...
  const filteredFurniture = furnitureItems.filter(item => {
    if (selectedRoom === "living-room") return item.category === "living";
    if (selectedRoom === "bedroom") return item.category === "bedroom";
    if (selectedRoom === "kitchen") return item.category === "kitchen";
    return false;
  });

  const totalCost = placedItems.reduce((acc, id) => {
      ...
      <CardContent className="space-y-6">
              ...
              return (
                ...
                >
                  ...
                  <Button
                    ...
                    aria-label={isPlaced ? `Remove ${item.name}` : `Add ${item.name}`}
        ...
        <div className="p-3 bg-warm-gray-50 rounded-lg">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="font-bold text-teal-600">${totalCost.toLocaleString()}</span>
          </div>
